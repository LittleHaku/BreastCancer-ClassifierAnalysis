...
good \ab{better keep it objective ;) -- 'more detailed'}
explanation of \ac{shap} is given in Section \ref{SEC:SHAP}.

\ab{was the shap library referenced anywhere? I am not following any link or anything, and right now I don't remember... If not, this is a good moment to include the link either as a footnote or as a reference}
...
This will allow us to understand why a certain prediction was made and which features were the most important for that prediction, to do this we \ab{there is some repetition in the last two sentences, it would be better to rewrite and/or summarise} 
will 
plot \ab{where? in the results? now? is your code able to plot more than 3 cases? take into account that you are now describing the implementation of your code, as I said before, it is much better to present this as generic as possible. But apart from that, this 'we will plot the prediction of 3 cases' it is confusing because it is not clear when this is plotted}
the prediction of three cases where the classifier was sure the sample was 
benign, three where it was sure it is benign, \ab{to be checked, as it says the same in both!}
the three closest to the decision boundary and 
the \ab{in this case it is also 3 examples or all the misclassified ones?}
